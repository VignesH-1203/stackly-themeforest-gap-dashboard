...
<style>
    ...
    .stMetric label, .stMetric [data-testid="stMetricValue"] { color: #FAFAFA; }
</style>
...
def priority_badge(priority: str) -> str:
    ...
    return f'<span style="background:{color};color:white;padding:3px 10px;border-radius:10px;font-size:12px;font-weight:600;">{priority}</span>'

def status_badge(status: str) -> str:
    return f'<span style="background:#374151;color:#FAFAFA;padding:3px 10px;border-radius:10px;font-size:12px;">{status}</span>'

def impact_badge(impact: str) -> str:
    color = "#10B981" if impact == "High" else "#6B7280"
    return f'<span style="background:{color};color:white;padding:3px 10px;border-radius:10px;font-size:12px;">{impact}</span>'
