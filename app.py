...
<style>
    ...
    /* Sidebar collapse / minimize arrow — keep visible on dark theme */
    ...
    .rec-table th, .rec-table td { color: #FAFAFA !important; }
    .rec-table th {
...
def get_recommendations() -> pd.DataFrame:
    # Aligned with Section 6 of the report (Table 15)
    return pd.DataFrame({
        "Recommendation": [
            "Fix or remove all broken links (social, login, policy)",
            "Create documentation & setup guide for each template",
            "Add SEO meta tags and Open Graph tags to every page",
            "Convert Fashion's .jpeg images to .webp and consolidate icon sources",
            "Add search bar and product filtering / sorting",
            "Increase homepage layouts and add more inner pages",
            "Build marketplace listing assets (previews, descriptions, feature lists)",
            "Test & document mobile responsiveness; implement lazy loading",
        ],
        "Applies To": [
            "All three", "All three", "All three", "Fashion",
            "Fashion, Minimal-Retail", "All three", "All three", "All three",
        ],
        "Priority": ["High", "High", "High", "High", "Medium", "Medium", "Medium", "Medium"],
        "Impact":   ["High", "High", "High", "High", "Medium", "High", "High", "Medium"],
...
with st.sidebar:
    ...
    selected_category = st.selectbox(
        "Category",
        ["All"] + CATEGORIES,
        help="Drill down into a specific category below the radar",
    )

    st.divider()
    st.caption("Sample data only · Internal review")
...
# Radar always shows all 5 categories so the polygon shape is preserved
radar_df = gap_df.copy()
...
radar_fig.update_layout(
    polar=dict(
        ...
        angularaxis=dict(gridcolor="#374151", tickfont=dict(color="#FFFFFF", size=12)),
        bgcolor="#1E1E2E",
    ),
    showlegend=True,
...
# Category drill-down — only shows when user picks a specific category
if selected_category != "All":
    st.markdown(f"**Drill-down — {selected_category} scores**")
    drill_cols = st.columns(len(selected_sites))
    cat_row = gap_df.set_index("Category").loc[selected_category]
    for col, site in zip(drill_cols, selected_sites):
        col.metric(site, f"{int(cat_row[site])}/100")
...
# Build text labels so users can read each cell without hovering.
# pandas promotes columns with any None to float, so coerce to int before lookup.
status_label = {0: "Missing", 1: "Partial", 2: "Present"}

def cell_label(v):
    if pd.isna(v):
        return "N/A"
    return status_label.get(int(v), "N/A")

text_matrix = [[cell_label(v) for v in row] for row in heat_df.values]
...
heat_fig.update_layout(
    height=600,
    paper_bgcolor="#0E1117",
    plot_bgcolor="#374151",  # grey shows through where N/A cells leave a gap
...
st.caption(
    "How far each template is from the marketplace standard. "
    "Page Load Time is in seconds (lower is better) — all other metrics are 0–100 scores (higher is better). "
    "Bar values are labelled above each bar for clarity."
)
...
tech_fig.update_layout(
    ...
    plot_bgcolor="#0E1117",
    font=dict(color="#FFFFFF", size=12),
    xaxis=dict(gridcolor="#374151", tickfont=dict(color="#FFFFFF", size=11)),
    yaxis=dict(gridcolor="#374151", title="Score / Value", tickfont=dict(color="#FFFFFF", size=11)),
    legend=dict(
        orientation="h",
...
comp_fig.update_layout(
    ...
    plot_bgcolor="#0E1117",
    font=dict(color="#FFFFFF", size=12),
    xaxis=dict(gridcolor="#374151", title="Total Pages", tickfont=dict(color="#FFFFFF", size=11)),
    yaxis=dict(gridcolor="#374151", tickfont=dict(color="#FFFFFF", size=12)),
...
st.caption("The eight recommendations from Section 6 of the gap analysis report — none started yet.")
